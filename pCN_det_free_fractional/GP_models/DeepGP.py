...
class DeepGP:
    def __init__(self, layers, layer_n_dof, F, alpha=4, base_diag=400, sigma=2.0):
        self.alpha = alpha
        self.base_layer = GPLayer(n_dof=layer_n_dof, alpha=alpha, sigma=sigma)
    ...
    def evaluate(self, random_sample, top_sample):
        ...
        u_prev = self.base_layer.evaluate(diag_vector, base_sample, fix_diag=True)
        ...
        for i in range(1, self.layers):
            ...
            u_prev = self.top_layer.evaluate(diag_vector, layer_sample, fix_diag=True)
        ...
        u_top = self.top_layer.evaluate(diag_vector, top_sample)
        
        return u, u_top, diag_vector
