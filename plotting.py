...
def plot_deep_gp_samples(u_list, layers, save_as):
    num_samples = len(u_list)
    layer_n_dof = int(u_list[0].size / layers)
    _, axs = plt.subplots(layers, num_samples, figsize=(num_samples * 5, layers * 4))

    for i, u in enumerate(u_list):
        for j in range(layers):
            idx_lower = (layers - j - 1) * layer_n_dof
            idx_upper = (layers - j) * layer_n_dof
            plot_2D_function(axs[j, i], u[idx_lower:idx_upper])
            if j == 0:
                axs[j, i].set_title(f"Sample {i + 1}")

    plt.savefig(save_as, bbox_inches="tight")


def plot_deep_gp_2D(u_list, save_path=None):
    ...
    for i, u in enumerate(u_list):
        ...
        ax = plt.gca()
        plot_2D_function(ax, u)
        if save_path:
            plt.savefig(save_path + f"u_{i}.pdf", bbox_inches="tight")
    
    print("Finished plotting.")


def plot_endless_deep_gp_2D(u_vals, save_as=None):
    print("Plotting results.")
    ...
    plt.figure(figsize=(len(u_vals) * 3.9, 3.9))
    u_merged = np.concatenate(u_vals, axis=1)
    plt.imshow(u_merged, cmap="viridis")
    plt.axis("off")
...
def plot_gp_list_2D(u_list, save_path=None):
    print("Plotting results.")
    ...
    for i, u in enumerate(u_list):
        ...
        plot_2D_function(ax, u)
        if save_path:
            plt.savefig(save_path + f"gp_sample_{i}.pdf", bbox_inches="tight")
    
    print("Finished plotting.")


def plot_convergence_results(stats, save_path=None):
    print("Plotting results...")
    ...
    for err_key in stats.keys():
        err_dict = stats[err_key]
        if err_key in ["ones", "random_rhs", "white"]:
            ylabel = r"relative $L^2$--error"
        elif err_key in ["potential_rel", "logdet_rel"]:
            ylabel = r"relative error"
        else:
            ylabel = r"absolute error"
        file_name = save_path + err_key + ".pdf"
        plot_single_convergence_result(err_dict, markers, ylabel, save_as=file_name)
    

def plot_single_convergence_result(err_dict, markers, ylabel, save_as=None):
    plt.figure(figsize=(3.9, 3.9))
    for i, n in enumerate(err_dict.keys()):
        if np.isscalar((list(err_dict[n].values())[0])):
            plt.plot(err_dict[n].keys(), err_dict[n].values(), label=rf"$n={n}$", marker=markers[i])
        else:
            k = err_dict[n].keys()
            vals_array = np.array(list(err_dict[n].values()))
            plt.plot(k, vals_array[:, 0], label=rf"$n={n}$", marker=markers[i])
            plt.fill_between(k, vals_array[:, 1], vals_array[:, 2], alpha=0.2)
